function RGBim = makeHype2RGB(hypIM,lambda,CMFname,IlluName,gamma)

% Converts the hyperspectral image into a RGB mapped image 
...
cmfIllu = cmf .* repmat(energy,1,3);

nonanvalues = ~isnan(cmfIllu);
XYZim = reshape(hypSP(:,nonanvalues(:,1))*cmfIllu(nonanvalues(:,1),:),xS,yS,[]);
RGBim = xyz2rgb(XYZim).^(gamma);
RGBim = RGBim - min(RGBim(:));
